...
const Globe = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const ringRadii = [0.18, 0.28, 0.4, 0.55, 0.72, 0.9, 1.1, 1.35];
      ...
      for (let t = 1; t < 20; t++) {
        ...
        ctx.strokeStyle = `rgba(255,255,255,${alpha_t})`;
        ctx.lineWidth = 0.5;
        const tx = cx + t * tickSpacing,
          txN = cx - t * tickSpacing;
        if (tx < w) {
          ctx.beginPath();
        ...
        if (txN > 0) {
          ctx.beginPath();
          ctx.moveTo(txN, cy - tickLen);
          ctx.lineTo(txN, cy + tickLen);
          ctx.stroke();
        }
        const ty = cy + t * tickSpacing,
          tyN = cy - t * tickSpacing;
        if (ty < h) {
          ctx.beginPath();
        ...
        if (tyN > 0) {
          ctx.beginPath();
          ctx.moveTo(cx - tickLen, tyN);
          ctx.lineTo(cx + tickLen, tyN);
          ctx.stroke();
        }
      }
      ...
      for (let i = 0; i < 12; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(
          cx + Math.cos(angle) * maxR * 0.22,
          cy + Math.sin(angle) * maxR * 0.22,
        );
        ctx.lineTo(
          cx + Math.cos(angle) * maxR * 1.2,
          cy + Math.sin(angle) * maxR * 1.2,
        );
      ...
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(w - bi - bs, bi);
      ctx.lineTo(w - bi, bi);
      ctx.lineTo(w - bi, bi + bs);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(bi, h - bi - bs);
      ctx.lineTo(bi, h - bi);
      ctx.lineTo(bi + bs, h - bi);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(w - bi - bs, h - bi);
      ctx.lineTo(w - bi, h - bi);
      ctx.lineTo(w - bi, h - bi - bs);
      ctx.stroke();
  ...
  useEffect(() => {
    ...
    const setupBaseLayers = (map) => {
      try {
        map.setFog(getFogConfig(activeLayerId));
      } catch {}
      ...
      if (!map.getLayer("conflict-country-fill")) {
        map.addLayer({
          ...
          paint: {
            ...
            "fill-opacity": [
              "case",
              ["boolean", ["feature-state", "conflict"], false],
              1,
              0,
            ],
          },
      ...
      map.on("click", (e) => {
        ...
        try {
          ...
          const f = map.queryRenderedFeatures(e.point, {
            layers: ["country-borders", "country-highlight-fill"],
          });
